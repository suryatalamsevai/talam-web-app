'use client'

import { DotPattern } from '@/components/ui/dot-pattern'
import { BlurFade } from '@/components/ui/blur-fade'

export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 text-center">
      <DotPattern glow={false} className="text-brand-primary/15 [mask-image:radial-gradient(360px_circle_at_center,white,transparent)]" />
      <BlurFade direction="up" className="relative space-y-3">
        <p className="font-heading text-6xl font-bold text-brand-primary/30">500</p>
        <h1 className="font-heading text-2xl font-bold text-fg">Something went wrong</h1>
        <p className="mx-auto max-w-sm font-body text-sm text-muted-foreground">
          An unexpected error occurred on our end. Please try again in a moment.
        </p>
        <button
          onClick={reset}
          className="mt-4 inline-flex h-10 items-center justify-center rounded-full bg-brand-primary px-6 font-body text-sm font-semibold text-white"
        >
          Try again
        </button>
      </BlurFade>
    </main>
  )
}
